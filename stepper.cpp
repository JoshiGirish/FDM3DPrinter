...
static long st_x, st_y, st_z, st_e;
...
ISR(TIMER1_COMPA_vect) {
  ...
  if (current_block != NULL) {
    ...
    for (int8_t i = 0; i < step_loops; i++) {
      ...
      #define A_HIGH(AXIS) SET_A_## AXIS
      #define B_HIGH(AXIS) SET_B_## AXIS
      #define C_HIGH(AXIS) SET_C_## AXIS
      #define D_HIGH(AXIS) SET_D_## AXIS
      #define A_LOW(AXIS) RESET_A_## AXIS
      #define B_LOW(AXIS) RESET_B_## AXIS
      #define C_LOW(AXIS) RESET_C_## AXIS
      #define D_LOW(AXIS) RESET_D_## AXIS
      
      #define _STAT(axis) st_## axis
      ...
      #define ENERGIZE_COILS(axis,AXIS) \
      if (_COUNTER(axis) > 0) { \
        _STAT(axis) += count_direction[_AXIS(AXIS)]; \
          if (_STAT(axis) > 4) { \
             _STAT(axis) = 1; \
          } \
          else if (_STAT(axis) < 1) { \
            _STAT(axis) = 4; \
          } \
               else { \
                    } \
...
void motor_initialize(){
  pinMode(X_a_pin, OUTPUT);
  pinMode(X_b_pin, OUTPUT);
  pinMode(X_c_pin, OUTPUT);
  pinMode(X_d_pin, OUTPUT);

  pinMode(Y_a_pin, OUTPUT);
  pinMode(Y_b_pin, OUTPUT);
  pinMode(Y_c_pin, OUTPUT);
  pinMode(Y_d_pin, OUTPUT);

  pinMode(Z_a_pin, OUTPUT);
  pinMode(Z_b_pin, OUTPUT);
  pinMode(Z_c_pin, OUTPUT);
  pinMode(Z_d_pin, OUTPUT);

  pinMode(E_a_pin, OUTPUT);
  pinMode(E_b_pin, OUTPUT);
  pinMode(E_c_pin, OUTPUT);
  pinMode(E_d_pin, OUTPUT);
